...
void ParticleController::repulseParticles()
{
	for( list<Particle>::iterator p1 = mParticles.begin(); p1 != mParticles.end(); ++p1 ){
		...
		for( ++p2; p2 != mParticles.end(); ++p2 ) {
			...
			float thresh = ( p1->mRadius + p2->mRadius ) * 3.0f;
...
void ParticleController::addParticles( int amt, const Vec2i &mouseLoc, const Vec2f &mouseVel )
{
	for( int i=0; i<amt; i++ )
	{
		...
		Vec2f velOffset = Rand::randVec2f() * Rand::randFloat( 1.0f, 2.0f );
//		Vec2f vel = mouseVel * 5.0f + velOffset;
//        Vec2f vel(1.5f, Rand::randFloat(1.0f, 200.0f));
//        vel = vel * Rand::randFloat( 1.0f, 200.0f );
        Vec2f vel = Rand::randVec2f() * Rand::randFloat( 1.0f, 200.0f );
//        Vec2f vel = Rand::randVec2f();
