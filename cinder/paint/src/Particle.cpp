...
Particle::Particle( Vec2f loc, Vec2f vel )
{
	...
	mRadiusDest		= 3.0f;
	mMass			= mRadius * mRadius * 0.01f + 0.01f;
	mScale			= 3.0f;
	mAge			= 0;
...
{	
	...
	if( mLoc.x >= 0.0f && mLoc.x <= (float)app::getWindowWidth() - 1 &&
        ...
        mChannel = channel.getValue( mLoc );
//        cinder::app::console() << "channel = " << channel.getValue(mLoc) << std::endl;
		
//		mRadiusDest	= channel.getValue( mLoc ) * 3.5f;
        mRadiusDest	= channel.getValue( mLoc ) * (Rand::randFloat(0.1f, 30.0f) * channel.getValue( mLoc ));
	} else {
		mRadiusDest = 0.1f;
	}
...
void Particle::draw()
{
    gl::color( ColorA( mChannel * 1.8f, mChannel * 1.8f, mChannel * 1.8f, mChannel) );
//    gl::color( ColorA( 1.0f, 1.0f, 1.0f, mChannel ) );
